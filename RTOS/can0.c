...
//uint32_t RCV_ID = 3; // set dynamically at time of CAN0_Open
//uint32_t XMT_ID = 2;
uint32_t RCV_ID = 2;
uint32_t XMT_ID = 3;
...
int static MailFlag;
Sema4Type CAN0Free;       // used for mutual exclusion
...
void static
CAN0_Setup_Message_Object(uint32_t MessageID, uint32_t MessageFlags,
        uint32_t MessageLength, uint8_t *MessageData, uint32_t ObjectID,
...
// Initialize CAN port
void
CAN0_Open(uint32_t rcvId, uint32_t xmtID) {
...
// in this case there is just one type, but you could accept multiple ID types
    ...
    OS_InitSemaphore(&CAN0Free, 1);
    return;
}

// send 4 bytes of data to other microcontroller 
void
CAN0_SendData(uint8_t data[4]) {
// in this case there is just one type, but you could accept multiple ID types
    OS_Wait(&CAN0Free);
...
int
CAN0_GetMailNonBlock(uint8_t data[4]) {
    if (MailFlag) {
        data[0] = RCVData[0];
        data[1] = RCVData[1];
        data[2] = RCVData[2];
