...
const OPT3101Speed_t OPT3101Choices_3[10] = { { 1, 10, 10 }, //   didn't work
        { 2, 9, 20 }, //   4.02ms
        { 4, 8, 40 }, //   7.04ms
        { 8, 7, 80 }, //   6.03ms
        { 16, 6, 160 }, //   8.05ms
...
uint32_t SUB_FRAME_COUNT_3 = 128;
...
uint32_t XTALK_FILT_TIME_CONST_3 = 3;
...
uint32_t CROSSTALK_SETTLING_TIME_MS_3 = 1280;

static uint32_t reg08_3, reg09_3;
//uint32_t r1,r2,d3;
uint32_t
...
//uint32_t OPT3101_ReadRegister_3(uint8_t address){
//  uint8_t buffer[3];
...
//}
void
OPT3101_WriteRegister_3(uint8_t address, uint32_t data) {
    I2C3_Send4(I2C_ADDRESS, address, data & 0xFF, (data >> 8) & 0xFF,
            (data >> 16) & 0xFF);
}
//void OPT3101_WriteRegister_3(uint8_t address, uint32_t data){
//  uint8_t buffer[] = {
//    address,
//    data & 0xFF,
//    data >> 8 & 0xFF,
...
OPT3101_3_Init(uint32_t speed) {
    ...
    SUB_FRAME_COUNT_3 = OPT3101Choices_3[speed].sub_frame_count;
    XTALK_FILT_TIME_CONST_3 = OPT3101Choices_3[speed].xtalk_filt_time_const;
    CROSSTALK_SETTLING_TIME_MS_3 =
            OPT3101Choices_3[speed].crosstalk_settling_time_ms;
...
OPT3101_3_Setup(void) {
    ...
    OPT3101_WriteRegister_3(0x89, 7000);

    // Enable the temperature sensor.
    // This choice comes from the OPT3101 Configurator Tool version 0.8.0.
    {
        uint32_t reg6e = OPT3101_ReadRegister_3(0x6e);
        reg6e |= 0x80000;  // EN_TEMP_CONV = 1
        OPT3101_WriteRegister_3(0x6e, reg6e);
    }

    // Turn on clip mode for frequency-correction to phase.
    // This choice comes from the OPT3101 Configurator Tool version 0.8.0.
    {
        uint32_t reg50 = OPT3101_ReadRegister_3(0x50);
        reg50 |= 1;
        OPT3101_WriteRegister_3(0x50, reg50);
    }

    // Set NUM_SUB_FRAMES to SUB_FRAME_COUNT - 1.
    // Set NUM_AVG_SUB_FRAMES to the same value so we are averaging together
    // every sub-frame.
    OPT3101_WriteRegister_3(0x9f,
            (uint32_t) (SUB_FRAME_COUNT_3 - 1) << 12 | (SUB_FRAME_COUNT_3 - 1));

    // Set XTALK_FILT_TIME_CONST to the corresponding value.
    {
        uint32_t reg2e = OPT3101_ReadRegister_3(0x2e);
        reg2e = (reg2e & ~0xF00000) | (uint32_t) XTALK_FILT_TIME_CONST_3 << 20;
        OPT3101_WriteRegister_3(0x2e, reg2e);
    }

    // Set up the GPIO1 pin (which is connected to P6.2/AUXR) to be
    // a data-ready signal.
    {
        uint32_t reg78 = OPT3101_ReadRegister_3(0x78);
        uint32_t reg0b = OPT3101_ReadRegister_3(0x0b);
        reg78 |= 0x1000;                   // GPIO1_OBUF_EN = 1
        reg78 = (reg78 & ~0x1C0) | 0x080;  // GPO1_MUX_SEL = 2 (DIG_GPO_0)
        reg0b = (reg0b & ~0xF) | 9;        // DIG_GPO_SEL0 = 9 (DATA_RDY)
        OPT3101_WriteRegister_3(0x78, reg78);
        OPT3101_WriteRegister_3(0x0b, reg0b);
    }

    {
        uint32_t reg2a = OPT3101_ReadRegister_3(0x2a);
        reg2a |= 0x8000;  // EN_ADAPTIVE_HDR = 1 : Adaptive HDR
        reg2a |= 1;       // EN_TX_SWITCH = 1    : Automatic channel-switching.
        OPT3101_WriteRegister_3(0x2a, reg2a);
//    Reg2a = OPT3101_ReadRegister_3(0x2a); // debugging
    }
    ...
    {
        uint32_t reg27 = OPT3101_ReadRegister_3(0x27);
        reg27 |= 3;                             // MONOSHOT_MODE = 3
        reg27 = (reg27 & 0xFFFF03) | (1 << 2);  // MONOSHOT_NUMFRAME = 1
        OPT3101_WriteRegister_3(0x27, reg27);
//    Reg27 = OPT3101_ReadRegister_3(0x27); // debugging

    }
    {
        uint32_t reg76 = OPT3101_ReadRegister_3(0x76);
        reg76 |= 0x001;  // DIS_GLB_PD_REFSYS = 1
        reg76 |= 0x020;  // DIS_GLB_PD_AMB_DAC = 1
        reg76 |= 0x100;  // DIS_GLB_PD_OSC = 1
        OPT3101_WriteRegister_3(0x76, reg76);
    }
    {
        uint32_t reg26 = OPT3101_ReadRegister_3(0x26);
        reg26 = (reg26 & 0x0003FF) | (95 << 10);  // POWERUP_DELAY = 95
        OPT3101_WriteRegister_3(0x26, reg26);
    }
}

void
OPT3101_3_CalibrateInternalCrosstalk(void) {
    // Clear TG_EN because the OPT3101 datasheet says EN_SEQUENCER should only be
    // changed while TG_EN is 0.
    uint32_t reg80 = OPT3101_ReadRegister_3(0x80);
    reg80 &= ~1;  // TG_EN = 0
    OPT3101_WriteRegister_3(0x80, reg80);

    uint32_t orig_reg2a = OPT3101_ReadRegister_3(0x2a);
    uint32_t orig_reg2e = OPT3101_ReadRegister_3(0x2e);
    ...
    uint32_t reg14 = OPT3101_ReadRegister_3(0x14);
    reg14 &= ~(1 << 16);     // EN_SEQUENCER = 0
    reg14 &= ~(1 << 17);     // EN_PROCESSOR_VALUES = 0
    OPT3101_WriteRegister_3(0x14, reg14);
    ...
    DisableInterrupts();
    Clock_Delay1ms(CROSSTALK_SETTLING_TIME_MS_3);
    EnableInterrupts();
    ...
    OPT3101_WriteRegister_3(0x2a, orig_reg2a);
    OPT3101_WriteRegister_3(0x2e, orig_reg2e);
...
OPT3101_3_StartMeasurementChannel(uint32_t ch) {
    if (ch <= 2) {
        ...
        OPT3101_WriteRegister_3(0x2a, reg2a); // Set MONOSHOT_BIT to 1 to trigger a new measurement.
    }
    // Assumption: The other bits in register 0 should be 0.
    OPT3101_WriteRegister_3(0x00, 0x800000);
}
void
OPT3101_3_ReadMeasurement(void) {
    reg08_3 = OPT3101_ReadRegister_3(0x08);
    reg09_3 = OPT3101_ReadRegister_3(0x09);
}

bool
OPT3101_3_MeasurementError(void) {
    if (!((reg08_3 >> 20) & 0x0001)) {
...
uint32_t
OPT3101_3_GetTxChannel(void) {
    return (reg08_3 >> 18) & 0x0003;  // TX_CHANNEL
}

uint32_t
OPT3101_3_GetAmplitude(void) {
    return reg09_3 & 0xFFFF;  // AMP_OUT
}
//
uint32_t
OPT3101_3_GetPhase(void) {
    return reg08_3 & 0xFFFF;  // PHASE_OUT
}

uint32_t
OPT3101_3_GetDistanceMillimeters(void) {
// return OPT3101_GetPhase() * MM_PER_PHASE_COUNT; // removed floating point
    return (OPT3101_3_GetPhase() * BinFixMM_PER_PHASE_COUNT) >> BinFix;
}
...
OPT3101_3_GetMeasurement(uint32_t distances[3], uint32_t amplitudes[3]) {
    ...
    distance = OPT3101_3_GetDistanceMillimeters();
    amplitude = OPT3101_3_GetAmplitude();
    channel = OPT3101_3_GetTxChannel();

    if (OPT3101_3_MeasurementError()) {
...
void
OPT3101_3_ArmInterrupts(uint32_t *pTxChan, uint32_t distances[3],
